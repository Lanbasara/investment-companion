...
TOOLS={
 ...
 "delivery_prepare":("为必报或摘要任务冻结用户结果；必报结果将进入可重试的 cc-connect 发送队列。",schema({"delivery_id":S,"conclusion":{"type":"string","enum":["no_action","action","risk_action","review_required","insufficient_evidence","system_degraded"]},"summary":S,"key_evidence":{"type":"array","items":S},"next_step":S,"next_check_at":S,"source_refs":{"type":"array","items":S}},["delivery_id","conclusion","summary","key_evidence","next_step"])),
 "delivery_digest_send":("把多个摘要任务的结果合并为一条用户消息并提交实际发送；只接受同一会话的 digest_required 记录。",schema({"delivery_ids":{"type":"array","items":S},"conclusion":{"type":"string","enum":["no_action","action","risk_action","review_required","insufficient_evidence","system_degraded"]},"summary":S,"key_evidence":{"type":"array","items":S},"next_step":S,"next_check_at":S,"source_refs":{"type":"array","items":S}},["delivery_ids","conclusion","summary","key_evidence","next_step"])),
