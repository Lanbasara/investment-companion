...
class V6PredictiveRecommendations:
    ...
    def _ensure_schedules(self, config: dict[str, Any], definitions: list[dict[str, Any]]) -> list[dict[str, Any]]:
        ...
        specifications=[
            {"role":"stock_candidates", "kind":"maintenance", "name":"V6 股票：研究候选扫描", "mission":"基于冻结的 V5 A 股日线与复权扫描生成独立 V6 股票候选；候选不是预测或交易。", "cadence":{"type":"local_time","at":"18:25","timezone":"Asia/Shanghai","weekdays":[0,1,2,3,4]}, "parameters":{"program_id":config["program_id"],"top_k":10}, "handler":STOCK_CANDIDATE_HANDLER},
            {"role":"stock_signals", "kind":"maintenance", "name":"V6 股票：冻结预测信号", "mission":"冻结可否证的股票趋势预测并自动结算未来结果；从首日给出暂定推荐或不推荐。", "cadence":{"type":"local_time","at":"18:35","timezone":"Asia/Shanghai","weekdays":[0,1,2,3,4]}, "parameters":{"program_id":config["program_id"],"horizon_sessions":20}, "handler":STOCK_SIGNAL_HANDLER},
