...
状态：券商事实基线已冻结（2026-08-25）；未知项继续保持未知。

本目录描述的是 `Decision -> ManualActionSpec -> 用户确认 -> 券商执行 -> 成交对账` 之间的执行工具箱。券商能力不会绕过研究验证、风险门、用户授权或确认账本，也不等同于推荐或成交。

系统中的 `valid_until` 必须逐字采用券商 App 展示的截止时刻，并与 Decision 截止时刻相同；`5/20/60/180` 只记录用户在券商选择的交易日档位。系统不自行猜测节假日日历来替代券商显示结果。

内容分级必须严格区分：

- `BrokerFacts`：用户提供的 App 原文和中金客服确认，可以进入券商语义契约；
- `Unknowns`：券商未说明或客服无法确认，系统不得补猜；
- `CompanionControls`：本系统为了资金、持仓和审计安全增加的约束，不是中金功能定义；
- `ExternalNotes`：网络旁证，只能帮助提出问题，不能覆盖 BrokerFacts。
...
### CompanionControls：系统建议规格（不是券商事实）
...
### CompanionControls：主要风险与系统约束（不是券商事实）
...
- 买方向休眠和卖方向休眠是两个独立状态，不能用整张网格暂停代替；
...
`terminated` 仅表示券商网格本体已经终止，不表示遗留委托已撤销，也不表示持仓已对齐。每个实际订单分别进入现有 `Execution -> pending Ledger -> 用户确认 -> confirmed Ledger` 真相链；只有遗留委托全部终态、相关成交全部确认入账，且同账户在终止后完成 full-scope matched 对账，计划才可标记为 `reconciled`。

### ExternalNotes：网络调研补充（不得进入冻结券商语义）
