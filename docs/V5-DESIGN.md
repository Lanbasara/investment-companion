...
V5 把“持续产生真实证据”和“扩大正式能力声明”拆开：

- 数据研究链在 G0 后持续运行、每天服务收盘复盘并产生 G1 数据资格证据；它没有试用到期或运行次数上限；
- Decision Support Beta 在 G0–G4、用户明确 opt-in 和到期时间下产生 G5 真实飞书/人工使用证据；
- Full Decision Support 仍要求 G5；
- G6 评估策略和系统的前向净价值，不阻止每日研究与人工决策，也不会因天数到达自动变成 Go。

中文含义：Gate 的 `no_go` 是“不能把当前数据或策略包装成已正式合格”，不是“系统不能在真实世界工作”。候选可随时进入完整研究和手工 Decision；月度复盘检验方法，不充当功能开关。
