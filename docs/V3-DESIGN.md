...
状态：V3 Core 已实现并发布；真实用户数据初始化与长期使用验证尚未完成。本文同时保存目标设计和后续阶段，具体实现差异以 [PROJECT-STATUS.md](PROJECT-STATUS.md) 为准。
日期：2026-08-13
...
状态标记：Phase 0–4 已交付首版确定性能力；Phase 5 仅有文件信息源、Source Health 与手工 Market Snapshot，专用结构化 Adapter 尚待真实需求；Phase 6 只完成 Doctor、备份恢复和初始化，90 天运行验证尚未开始。

### Phase 0：契约冻结与测试基座（已交付首版）
...
### Phase 1：Personal Financial Kernel（已交付首版）
...
### Phase 2：Versioned Personal Context（已交付首版）
...
### Phase 3：Cognitive Lifecycle（已交付首版）
...
### Phase 4：Attention & Active Decision Loop（已交付首版，待真实消息验证）
...
V3 Core 已整体发布；Investor、Mandate 和 Financial Facts 仍以用户确认为启用门槛。未来版本不得把“代码存在”当作“长期验证完成”。

## 19. V3 目标完成定义

以下是长期产品目标，不等同于当前发布验收。V3.0 Core 已满足其中的确定性机制部分；真实对账、历史 Decision 重放、主动消息质量和长期恢复仍需用户数据及运行时间验证：
...
全部条件经过真实数据验证后，系统才从“V3 Core”进入真正可以长期托付的个人投资伴侣；未满足项应进入未来 V3.x/V4，而不是在文档中假定完成。
