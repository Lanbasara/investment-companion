...
- 先以 systemd timer 默认每30分钟调用 `companion tick`；调度器只负责到期扫描和兜底恢复，不承载每项 Watch 语义。普通新闻/文章巡视默认交易日收盘后一次，重点主题最多盘前、午间、收盘后各一次；临时高关注任务必须显式启用并带TTL。需要低延迟的推送入口在持久化后主动触发合并执行，不依赖提高全局轮询频率。
