...
## ADR-011：持续真实运行产生 Gate 证据，正式声明消费 Gate 证据

**决策**：量化数据研究在 G0 后持续运行、每天参与复盘并按月严格前向评估；不设试用到期或最大运行次数。它仍保持冻结端点、请求预算、用户批准和人工执行边界。Decision Beta 可另行采用有期限的 opt-in；Full Feature 仍要求对应 Gate。

**结果**：G1/G5 可以由真实使用产生，不形成逻辑死锁；证据不足降低结论强度而不关闭日常功能，方法无效时发布新版本或停止该基线，失败证据永久保留。
