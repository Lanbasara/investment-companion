...
- [x] 增加限时 Trial、请求预算、Schedule 到期和最大次数保护；
- [x] 增加真实数据回填、失败修复、前向观测和实验状态入口；
- [x] 明确禁止 Handler 自动创建 Opportunity、Decision、Shadow、Execution 或 Ledger Entry；
- [x] 更新 MCP、CLI、Plugin 与真实使用验收文档；
- [ ] 在固定生产 Runtime 上完成 Tushare 首轮回填、扫描和飞书唤醒 smoke；
- [ ] 经过 10–20 个真实交易日后，由 Primary 和用户共同决定继续、换版或停止。

退出条件：工程只证明链路可靠和边界有效；策略价值仍必须由真实前向样本证明。
