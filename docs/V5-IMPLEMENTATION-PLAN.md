...
- [x] 冻结首个 A 股日线/复权因子/交易日历数据契约；
- [x] 实现零模型 Token、输入可重放的确定性横截面扫描；
- [x] 增加单 Job 请求预算、真实数据回填、失败修复和前向观测；
- [x] 取消试用到期与最大运行次数，增加旧配置/历史无损迁移；
- [x] 增加候选持续性、当日复盘输入和完整研究触发；
- [x] 增加每月 19 日的确定性严格前向复盘及强制用户报告；
- [x] 明确禁止 Handler 自动创建 Opportunity、Decision、Shadow、Execution 或 Ledger Entry；
- [x] 更新 MCP、CLI、Plugin 与真实使用验收文档；
- [x] 在固定生产 Runtime 上完成 Tushare 首轮回填、扫描和飞书唤醒 smoke；
- [ ] 持续按日使用，并由每月复盘建议继续、深化、换版或停止。

验收原则：工程只证明链路可靠和边界有效；策略价值由持续真实前向样本检验，但样本积累不阻塞系统当天提供研究与人工决策服务。
