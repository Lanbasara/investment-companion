...
人工维护账户把精度拆成三条轴：matched 全量对账提供锚点；用户可确认锚点后无漏报并承诺持续报告，以维持 Ledger 数量与现金的连续性；最新行情独立刷新估值。对账日期变旧不会自动使数量失效，也不能成为拒绝投资判断的理由。缺少连续性确认时仍输出方向、范围和条件化数量；存在对账差异、待确认流水或开放执行时阻断最终精确数量。任何最终委托仍须在券商 App 核对可用现金与可用持仓。
